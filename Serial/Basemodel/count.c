...
/*
 * This function count_Zombies, counts the number of Zombie neighbours for a given index on the 2D array
 * 
 */
...
long long int count_Zombies(long long int i, long long int j) {
   ...
   for (x = i - 1; x <= i + 1; x++) // for loop to get rows of neighbours
   {
      for (y = j - 1; y <= j + 1; y++) // for loop to get columns of neighbours
      {
         if (current[(x + rows) % rows][(y + columns) % columns] == 0) //the torus is set here using modulus and if current = 0 then add 1 to the zombie count
            zombies++;
